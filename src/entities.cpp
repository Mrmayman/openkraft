...
void Entity::draw()
{
    ...
    glVertex2f(x-0.5f, y-0.5f); // Bottom-left vertex
    glVertex2f(x+0.5f, y-0.5f);  // Bottom-right vertex
    glVertex2f(x+0.5f, y+0.5f);   // Top-right vertex
    glVertex2f(x-0.5f, y+0.5f);  // Top-left vertex
...
void EntityPlayer::tick()
{
    ...
    if(rotationY > rylimit) { rotationY = rylimit; }
    if(rotationY < -rylimit) { rotationY = -rylimit; }
